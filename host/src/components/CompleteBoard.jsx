...
const CompleteBoard = ({ selectedNumbers = [], currentNumber = null, onNumberClick = null }) => {
  ...
  const getNumberClass = (number) => {
    ...
    const baseClass = `w-12 h-12 rounded-lg border-2 flex items-center justify-center font-bold text-sm transition-all duration-200 ${onNumberClick ? 'cursor-pointer' : 'cursor-default'}`
    
    switch (status) {
      case 'current':
        return `${baseClass} bg-yellow-500 text-black border-yellow-400 shadow-lg animate-pulse`
      case 'selected':
        return `${baseClass} bg-gray-700 text-green-400 border-green-600`
      case 'available':
        return `${baseClass} bg-gray-800 text-gray-300 border-gray-700 ${onNumberClick ? 'hover:bg-gray-700' : ''}`
      default:
    ...
    <div className="card p-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold flex items-center gap-2">
          ...
          Complete Tambola Board (1-90)
        </h3>
        <div className="text-sm text-gray-400">
          Selected: {selectedNumbers.length}/90
        </div>
      </div>
